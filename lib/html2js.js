...
var SINGLE_MODULE_TPL = "module.run(['$templateCache', function($templateCache) {\n" +
  "  $templateCache.put('%s',\n    '%s');\n" +
  '}]);\n';
  ...
  return function (content, file, done) {
    ...
    for (i = 0; i < templates.length; i++) {
      ...
      if (angular === 2 || angular === '2') {
        templates[i] = util.format(ANGULAR2_TPL, htmlPath, escapeContent(content))
      } else {
        if (moduleName) {
        ...
        if (enableRequireJs) {
          templates[i] = util.format(REQUIRE_MODULE_TPL, requireJsAngularId, templates[i])
        }
      }
    }

    if (angular === 2 || angular === '2' || !moduleName) {
      done(templates.join('\n'));
    } else {
